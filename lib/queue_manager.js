...
exports.chat_user_disconnect = function chat_user_disconnect(socketId){
 
	console.log(chat_users_list[socketId]);
	...
	//Was the user in a chat?
	if(user_in_chat){
		console.log("PLayer has dsiconnected from an active chat ")
...
exports.create_debate_room = function create_debate_room(debate_name){
	...
	for (i = 0; i <= 1; i++) {//get sockets of first 2 people in queue  
    	...
    	//Set user to "in chat"
 		chat_users_list[current_socket_id].in_chat = true;  
   	}
