			...
			if(err){
				...
				users.find({ '_id': ObjectId(req.user.id)}).toArray(function(err,result){
					if (err){
						...
						var testval = db_query.get_bookmarked_debates(result[0].debate_bookmarks, function(result){
						...
						console.log("test " , testval)
							...
							res.render("profile",{'user_data':result,'admin':req.user.admin,"bookmarked_debate":bookmarked_debates}); //the response data can be changed
